...
- For CISA KEV normalization or MCP release changes, preserve full-catalog CVE identity, provenance, and `forensicTriage` parity. Run `node --test scripts/test/mcp-proxy.spec.mjs scripts/test/verify_cisa_candidate.spec.mjs`; an authorized CISA-only release must bind the exact reviewed `expected_sha`, verify both the isolated candidate and canonical MCP endpoint with `node scripts/verify_cisa_candidate.mjs <exact-mcp-endpoint>`, require matching catalog and eligible-CVE digests plus selected identity, and restore captured traffic if promotion or parity fails.
