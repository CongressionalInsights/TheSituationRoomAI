...
- When changing EIA proxy, static-cache, or public-payload handling, preserve credential-field sanitization across Feed Proxy, MCP, monitor serialization, and public verification, and keep static publication fail-closed through the server-side Feed Proxy; run `node --test scripts/test/feeds.spec.mjs scripts/test/mcp-proxy.spec.mjs scripts/test/monitor.spec.mjs` and `node scripts/verify_public.mjs`.
